...
public class AutoLayoutGen : IIncrementalGenerator
{
    ...
    {
        // Define the pipeline for class declarations with ViewController attribute
        IncrementalValuesProvider<ViewModelClassInfo> viewControllerClassesValueProvider = context.SyntaxProvider
            .CreateSyntaxProvider(
                predicate: static (s, _) => IsClassWithAttributes(s),
                transform: static (ctx, _) => GetViewControllerClass(ctx))
            .Where(static m => m is not null)
            .Select(static (m, _) => m!);

        // Combine compilation with the values provider
        IncrementalValueProvider<(Compilation Left, ImmutableArray<ViewModelClassInfo> Right)> combinedProvider = context.CompilationProvider
            .Combine(viewControllerClassesValueProvider.Collect());

        // Register the source output
        context.RegisterSourceOutput(combinedProvider, (sourceProviderContext, source) =>
        {
            (Compilation compilation, ImmutableArray<ViewModelClassInfo> viewModelClasses) = source;
            Execute(sourceProviderContext, compilation, viewModelClasses);
        });
    }
    ...
    private static ViewModelClassInfo? GetViewControllerClass(GeneratorSyntaxContext context)
    {
        ...
        AttributeSyntax? viewControllerAttribute = classDeclaration
            .AttributeLists
            .SelectMany(lists => lists.Attributes)
            .FirstOrDefault(attribute => TestAttributeName(attribute, "ViewController"));

        if (viewControllerAttribute is null)
            return null;

        if (context.SemanticModel.GetDeclaredSymbol(classDeclaration) is not INamedTypeSymbol viewControllerSymbol)
            return null;

        Dictionary<IFieldSymbol, AttributeData> fieldDictionary = new(SymbolEqualityComparer.Default);
        Dictionary<string, CommandInfo> methodDictionary = [];

        foreach (ISymbol memberSymbol in viewControllerSymbol.GetMembers())
        {
            if (memberSymbol is IFieldSymbol fieldSymbol)
            {
                AttributeData? fieldViewControllerMappingAttribute = fieldSymbol.GetAttributes()
            ...
            if (memberSymbol is IMethodSymbol methodSymbol)
            {
                ...
                {
                    AttributeData? commandMappingAttribute = methodSymbol.GetAttributes()
                    ...
                    {
                        string baseLineName;
                        const string CanExecute = nameof(CanExecute);
                        const string Execute = nameof(Execute);
                        ...
                        if (methodSymbol.Name.StartsWith(CanExecute))
                        {
                            ...
                            {
                                baseLineName = methodSymbol.Name[CanExecute.Length..];
                                CommandInfo commandInfo = GetOrAddCommandInfo(methodDictionary, commandMappingAttribute, baseLineName);
                        ...
                        if (methodSymbol.Name.StartsWith(Execute))
                        {
                            ...
                            {
                                baseLineName = methodSymbol.Name[Execute.Length..];
                                CommandInfo commandInfo = GetOrAddCommandInfo(methodDictionary, commandMappingAttribute, baseLineName);
    ...
    private static bool TestAttributeName(AttributeSyntax attribute, string name)
    {
    ...
    private static CommandInfo GetOrAddCommandInfo(Dictionary<string, CommandInfo> methodDictionary, AttributeData commandMappingAttribute, string baseLineName)
    {
        if (methodDictionary.TryGetValue(baseLineName, out CommandInfo? commandInfo))
        {
            return commandInfo;
        }
        else
    ...
    private void Execute(SourceProductionContext context, Compilation compilation, ImmutableArray<ViewModelClassInfo> viewModelClasses)
    {
        ...
        try
        {
            foreach (ViewModelClassInfo viewModelItem in viewModelClasses)
            {
                SemanticModel semanticModel = compilation.GetSemanticModel(viewModelItem.SyntaxTree);
                INamedTypeSymbol? viewControllerSymbol = (INamedTypeSymbol)semanticModel.GetDeclaredSymbol(viewModelItem.ClassDeclaration)!;
                INamespaceSymbol? viewControllerNamespace = viewControllerSymbol?.ContainingNamespace;
                IEnumerable<IPropertySymbol>? formsControllerProperties = viewControllerSymbol?.GetMembers().OfType<IPropertySymbol>();

                Dictionary<IFieldSymbol, AttributeData> formsControllerFields = viewModelItem.FieldDefinitions;
                ...
                bindingExtensionClass.AppendLine($"{{");
                bindingExtensionClass.AppendLine($"    public partial class {viewModelItem.ClassDeclaration.Identifier.Text}");
                bindingExtensionClass.AppendLine($"    {{");
                ...
                foreach (KeyValuePair<string, CommandInfo> commandMethodInfo in viewModelItem.CommandMethodDefinitions)
                {
                    ...
                    if (commandMethodInfo.Value.FieldSymbol is null)
                    {
                        GenerateNotifyChangedProperty(
                            cacheTypeName,
                            compilation.GetTypeByMetadataName("System.Windows.Input.ICommand")!,
                ...
                foreach (KeyValuePair<IFieldSymbol, AttributeData> fieldAttributeTuple in formsControllerFields!)
                {
                    PropertyMappingAttribute mappingAttribute = GetPropertyMappingAttribute(fieldAttributeTuple.Key.Name, fieldAttributeTuple.Value);
                ...
                string viewModelClassString = viewModelClass.ToString();
                string extensionClassString = extensionClass.ToString();
        ...
        string? forcedTextPropertyContentInExtensionMethod=default)
    {
        ...
        if (autoLayoutTarget == AutoLayoutTarget.Label)
        {
            ...
            {
                extensionClassSourceCode.AppendLine($"{In2}        AutoLayoutLabel<{vmTypeName}> label = new(name: \"{propertyName}Label\", text: \"{defaultValueAssignment}\", bindings: {vmTypeName}.To{propertyName}(nameof(AutoLayoutTextEntry<{vmTypeName}>.Text)));");
            }
            else
            {
                extensionClassSourceCode.AppendLine($"{In2}        AutoLayoutLabel<{vmTypeName}> label = new(name: \"{propertyName}Label\", bindings: {vmTypeName}.To{propertyName}(nameof(AutoLayoutTextEntry<{vmTypeName}>.Text)));");
            }
        ...
        if (autoLayoutTarget == AutoLayoutTarget.TextEntry)
        {
            ...
            extensionClassSourceCode.AppendLine($"{In2}    int row, int column, int rowSpan = 1, int columnSpan = 1)");
            extensionClassSourceCode.AppendLine($"{In2}    {{");
            extensionClassSourceCode.AppendLine($"{In2}        AutoLayoutTextEntry<{vmTypeName}> textEntry = new(name: \"{propertyName}TextEntry\",");
            extensionClassSourceCode.AppendLine($"{In2}            bindings: {vmTypeName}.To{propertyName}(nameof(AutoLayoutTextEntry<{vmTypeName}>.Text)));");
            extensionClassSourceCode.AppendLine($"");
        ...
        if (autoLayoutTarget == AutoLayoutTarget.Button)
        {
            ...
            extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}(nameof(AutoLayoutButton<{vmTypeName}>.Command)),");
            extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}Caption(nameof(AutoLayoutButton<{vmTypeName}>.Text)));");
            extensionClassSourceCode.AppendLine($"");
        ...
        if (autoLayoutTarget == AutoLayoutTarget.DetailsList && propertyType.Name.StartsWith("ObservableCollection"))
        {
            if (string.IsNullOrWhiteSpace(forcedTextPropertyContentInExtensionMethod))
            {
                forcedTextPropertyContentInExtensionMethod = "Not defined.";
            ...
            ITypeSymbol? typeArgument = (propertyType as INamedTypeSymbol)?.TypeArguments.FirstOrDefault();
            string? obColGenericTypeName = typeArgument?.Name;

            if (obColGenericTypeName != null)
            {
                extensionClassSourceCode.AppendLine($"{In2}public static AutoLayoutGrid<{vmTypeName}> Add{propertyName}DataGrid(");
                extensionClassSourceCode.AppendLine($"{In2}    this AutoLayoutGrid<{vmTypeName}> grid,");
                extensionClassSourceCode.AppendLine($"{In2}    AutoLayoutSchema<{obColGenericTypeName}> autoLayoutSchema,");
                extensionClassSourceCode.AppendLine($"{In2}    int row, int column, int rowSpan = 1, int columnSpan = 1)");
                extensionClassSourceCode.AppendLine($"{In2}    {{");
                extensionClassSourceCode.AppendLine($"{In2}        AutoLayoutTableView<{vmTypeName},{obColGenericTypeName}> tableView = new(autoLayoutSchema: autoLayoutSchema, name: \"{propertyName}TableView\", null,");
                extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}(nameof(AutoLayoutTableView<{vmTypeName},{obColGenericTypeName}>.DataSource)),");
                extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}Caption(nameof(AutoLayoutTableView<{vmTypeName},{obColGenericTypeName}>.Text)));");
                extensionClassSourceCode.AppendLine($"");
                extensionClassSourceCode.AppendLine($"{In2}        grid.AddComponent(tableView, row, column, rowSpan, columnSpan);");
                extensionClassSourceCode.AppendLine($"");
                extensionClassSourceCode.AppendLine($"{In2}        return grid;");
                extensionClassSourceCode.AppendLine($"{In2}    }}");
                extensionClassSourceCode.AppendLine($"");
        ...
        if (autoLayoutTarget == AutoLayoutTarget.MenuItem)
        {
            ...
            extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}(nameof(AutoLayoutMenuItem<{vmTypeName}>.Command)),");
            extensionClassSourceCode.AppendLine($"{In2}            {vmTypeName}.To{propertyName}Caption(nameof(AutoLayoutMenuItem<{vmTypeName}>.Text)));");
            extensionClassSourceCode.AppendLine($"");
